...
//   ? >>> $set:
// & WAQ to update bobby perecharlas attendance to 92
db.students.updateOne({name: "Bobby Perecharla"},{$set: {attendance: 92}});
...
db.students.updateMany({"address.city" : "Chennai"},{$set: {enrolled: true}});


// ? >>> $unset:
// & WAQ to remove the phone number of Akash Kumar.
db.students.updateOne({ name: "Akash Kumar" },{ $unset: { "contact.phone": false } }
  );
  
  // & WAQ to remove the phone number of Sushant Hegde.
  db.students.updateOne({name: "Sushant Hegde"},{$unset: {"contact.phone": false}});
...
db.students.updateMany({},{ $inc: { "scores.midterm": -10, "scores.final": 2 } });
...
db.students.updateMany( {},{ $mul: { "scores.midterm": 0.8, "scores.final": 0.68 } });
