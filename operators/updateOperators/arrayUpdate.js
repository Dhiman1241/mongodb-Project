  ...
  */
  ...
  db.trainers.updateOne({trainerName: "Anjali Verma"},{$pull : {expertise: "Illustrator"}},{});
  ...
  db.trainers.updateOne({trainerName: "Anjali Verma"},{$set : {"students.0.name" : "Raj"}});
  
  // & WAQ to update the second expertise item in Amit’s expertise array to "Email Marketing".
  db.trainers.updateOne({trainerName: "Amit Patel"},{$set: {"expertise.1" : "Email Marketing"}},{});
  
  // & WAQ to update the third student's age in Priya’s students array to 26.
  db.trainers.updateOne({trainerName: "Priya Singh"},{$set: {"students.2.age" : 26}});
  ...
  db.trainers.updateOne({trainerName: "Rajesh Kumar"},{$pull: {students : {progress: "Intermediate"}}});
  // & Remove all students aged 24 or below from Priya's students array.
  db.trainers.updateOne({trainerName: "Priya Singh"},{$pull: {students: {age: 24}}});
  ...
  db.trainers.updateOne({trainerName: "Amit Patel"},{$pull: {students: {name: "Kiran"}}});
  // & Remove all expertise entries labeled "SQL" from Priya's expertise array.
  db.trainers.updateOne({trainerName: "Priya Singh"},{$pull: {expertise: "SQL"}});
  // ! === $pullAll ===
  ...
  db.trainers.updateOne({trainerName: "Amit Patel"},{$pullAll: {expertise: ["SEO", "Content Strategy"]}});
  // & Remove feedback scores of 5 and 4 from Anjali's feedbackScores array.
  db.trainerName.updateOne({trainerName: "Anjali Verma"},{$pullAll: {feedbackScores: [5,4]}});
  // & Remove all occurrences of 3 and 4 from Amit's feedbackScores array.
  db.trainerName.updateOne({trainerName: "Amit Patel"}, {$pullAll: {feedbackScores: [3,4]}});